...
class SobolAirfoilGenerator:
    ...
    def __init__(
        self, state_file="airfoil_state.json", seed=42, dat_path="../cleaned_foils/"
    ):
        self.state_file = state_file
        self.seed = seed
        self.dim = (
            self.BASE_DIMS + self.MAX_EXTRA_AIRFOILS * self.DIMS_PER_EXTRA_AIRFOIL
        )  # total number of required numbers generated
    ...
    def generate(self):
        ...
        if n == 3:  # this is kinda bad code
            ...
            for i in range(1, 3):  # i = 1 (second foil), i = 2 (third foil)
                ...
                chord_distr = [[0.25, 0.04, 0.18, 0.38], [0.15, 0.02, 0.1, 0.22]]

                generated_chord = (
                    chord_distr[i - 1][0]
                    + standard_normal_sample * chord_distr[i - 1][1]
                )  # normal distr
                chords[i] = float(
                    np.clip(
                        generated_chord, chord_distr[i - 1][2], chord_distr[i - 1][3]
                    )
                )  # clip between min/max
                ...
                if i == 1:
                    ...
                    gap_r = (
                        chords[i - 1] * gap_r_percent
                    )  # gap behavior more based on percent of length than raw distance
                elif i == 2:
                    gap_r_percent = 0.03 + 0.02 * u_gap_r
                    gap_r = chords[i - 1] * gap_r_percent
                gap_theta = np.radians(
                    -120 + 120 * u_gap_theta
                )  # includes overlap region
                gaps[i - 1] = [gap_r * math.cos(gap_theta), gap_r * math.sin(gap_theta)]
        else:  # n = 2, different scaling
            u_chord, u_defl, u_gap_r, u_gap_theta = u[
                self.BASE_DIMS : self.BASE_DIMS + 4
            ]
            standard_normal_sample = norm.ppf(u_chord)
